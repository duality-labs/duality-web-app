    ...
    return {
      ...
      columns: [
        ...
        function TokenCellRange({ row }: { row: Gauge }) {
          if (row.distributeTo?.pairID) {
            const Token0 = tokens.find(
              matchTokenByAddress(row.distributeTo.pairID.token0)
            );
            const Token1 = tokens.find(
              matchTokenByAddress(row.distributeTo.pairID.token1)
            );
            return (
              <td>
                {row.distributeTo && Token0 && Token1 && (
                  <div className="row gap-2">
                    <div className="col ml-auto">
                      {formatPrice(
                        tickIndexToPrice(
                          new BigNumber(`${row.distributeTo.startTick}`) || 0
                        ).toNumber(),
                      ...
                      {formatPrice(
                        tickIndexToPrice(
                          new BigNumber(`${row.distributeTo.endTick}`) || 0
                        ).toNumber(),
                        {
                          useGrouping: true,
              ...
              {row.coins.map((coin) => {
                ...
                return token ? (
                  <div key={coin.denom} className="row gap-2">
                    <div className="col ml-auto">
                      {formatAmount(
                        Number(coin.amount) / Number(row.numEpochsPaidOver)
                      )}
                    </div>
